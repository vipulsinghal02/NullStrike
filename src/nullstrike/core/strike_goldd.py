...
def strike_goldd(*args, model_name_override=None):
    ...
    if len(args) == 1:
        ...
        if hasattr(options, 'modelname') and options.modelname == '' and model_name_override:
            ...
            actual_model_name = options.modelname
            
        model = importlib.import_module('custom_models.{}'.format(actual_model_name))
        print(' Analyzing the {} model... \n'.format(actual_model_name))
    ...
    file = open(str(RESULTS_DIR / "id_results_{}_{}.txt".format(options.modelname, datetime.today().strftime('%d-%m-%Y'))), "w")
    file.write('\n RESULTS SUMMARY:')
